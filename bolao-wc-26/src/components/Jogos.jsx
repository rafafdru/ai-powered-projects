...
function CardJogo({ jogo, palpite, jaFoiSalvo, salvando, foiSalvoAgora, onSalvar, onChange, resultado, palpitesDoGrupo, virado, onVirar, usuarioAtual }) {
  ...
  // Montar lista de palpites do grupo para o verso
  ...
  const bordaFrente = foiSalvoAgora
  ...
  return (
    <div style={{ marginBottom: '10px', perspective: '1000px' }} onClick={(e) => {
      // só vira se clicar no container, não em inputs ou botões
      ...
      {!bloqueado && (
        ...
        <div style={{
          ...
          <div style={{ padding: '12px 14px 10px', borderBottom: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
            ...
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '14px', fontWeight: 900, color: '#fff', letterSpacing: '0.5px' }}>{jogo.time1.toUpperCase()} × {jogo.time2.toUpperCase()}</div>
          </div>

          {/* Lista de palpites — scrollável */}
          <div style={{ overflowY: 'auto', flex: 1, padding: '8px 0', maxHeight: '180px' }}>
            {listaPalpitesGrupo.length === 0 ? (
              <div style={{ padding: '16px 14px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '11px', color: 'rgba(255,255,255,0.2)', letterSpacing: '1px', textAlign: 'center' }}>NENHUM PALPITE AINDA</div>
            ) : (
              listaPalpitesGrupo.map(({ apelido, palpite: pal }) => {
                const isVoce = apelido === usuarioAtual;
                const temPal = pal && pal.g1 !== '' && pal.g2 !== '';
                return (
                  <div key={apelido} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 14px', background: isVoce ? 'rgba(201,168,76,0.06)' : 'transparent' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: isVoce ? 'linear-gradient(135deg, #C9A84C, #a8752a)' : 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '11px', fontWeight: 900, color: isVoce ? '#000' : 'rgba(255,255,255,0.4)', flexShrink: 0 }}>
                        {apelido[0].toUpperCase()}
                      </div>
                      <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '13px', fontWeight: 700, color: isVoce ? '#C9A84C' : 'rgba(255,255,255,0.6)', letterSpacing: '0.3px' }}>
                        {apelido.toUpperCase()}{isVoce && <span style={{ fontSize: '9px', color: 'rgba(201,168,76,0.4)', marginLeft: '4px' }}>VOCÊ</span>}
                      </span>
                    </div>
                    <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '15px', fontWeight: 900, color: isVoce ? '#C9A84C' : 'rgba(255,255,255,0.35)', letterSpacing: '2px' }}>
                      {temPal ? `${pal.g1} × ${pal.g2}` : <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.2)' }}>— × —</span>}
                    </span>
                  </div>
                );
              })
          ...
          {/* Rodapé do verso */}
          <div style={{ padding: '8px 14px', borderTop: '1px solid rgba(255,255,255,0.04)', flexShrink: 0, textAlign: 'center' }}>
            <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '9px', fontWeight: 700, letterSpacing: '2px', color: 'rgba(255,255,255,0.15)' }}>TOQUE PARA VOLTAR</span>
          </div>
        </div>{/* fim verso */}

      </div>{/* fim flip container */}
    </div>{/* fim perspective container */}
  );
}
...
export default function Jogos({ usuario, palpites, onSalvar, resultados, palpitesDoGrupo = {} }) {
      ...
      <div style={{ padding: '14px 14px 0' }}>
        {jogosOrdenados.map((jogo) => (
          <CardJogo
            ...
            onVirar={() => setCardVirado(cardVirado === jogo.id ? null : jogo.id)}
            usuarioAtual={usuario.apelido}
          />
        ))}
      </div>
