...
function App() {
  ...
  const [palpitesDoGrupo, setPalpitesDoGrupo] = useState({});
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      ...
      // palpites individuais de cada participante do grupo (para o verso do card)
      const palpitesGrupoMap = {};
      colGrupos.forEach((d) => {
        const id = d.id;
        if (!id.startsWith(prefixo)) return;
        const apelido = id.slice(prefixo.length);
        if (!apelido) return;
        palpitesGrupoMap[apelido] = d.data();
      });
      setPalpitesDoGrupo(palpitesGrupoMap);
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh', overflowX: 'hidden', width: '100%' }}>
      {tela === 'jogos' && (
        <Jogos usuario={usuario} palpites={palpites} onSalvar={handleSalvar} resultados={resultados} palpitesDoGrupo={palpitesDoGrupo} />
      )}
      {tela === 'matamata' && (
        <MataMata
