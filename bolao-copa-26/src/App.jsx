...
const IconPalpites = ({ ativo }) => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
    <rect
      ...
      height="7"
      rx="1.5"
      stroke={ativo ? '#C9A84C' : 'rgba(255,255,255,0.3)'}
      strokeWidth="1.5"
    />
    <rect
      x="12"
...
const IconRanking = ({ ativo }) => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
    <path
      d="M11 3 L13.5 8.5 L19.5 9.2 L15 13.5 L16.2 19.5 L11 16.5 L5.8 19.5 L7 13.5 L2.5 9.2 L8.5 8.5 Z"
      stroke={ativo ? '#C9A84C' : 'rgba(255,255,255,0.3)'}
      strokeWidth="1.5"
      strokeLinejoin="round"
    />
...
function App() {
  ...
  useEffect(() => {
    try {
      ...
      const ativaSalva = parseInt(
        localStorage.getItem(LS_CONTA_ATIVA) ?? '-1',
        10
      );
      if (
        contasSalvas.length > 0 &&
        ativaSalva >= 0 &&
        ativaSalva < contasSalvas.length
      ) {
  ...
  const usuario =
    contaAtiva !== null && contas[contaAtiva] ? contas[contaAtiva] : null;
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      ...
      const mmRef = doc(
        db,
        'palpitesMataMata',
        `${usuario.codigo}_${usuario.apelido}`
      );
      ...
      colGrupos.forEach((d) => {
        ...
        pontosMap[apelido] =
          (pontosMap[apelido] ?? 0) +
          calcularPontos(d.data(), resultadosAtuais);
      });
      ...
      const lista = Object.entries(pontosMap).map(([apelido, pontos]) => ({
        apelido,
        pontos,
      }));
  ...
  async function handleSalvar(jogoId, palpite) {
    ...
    await setDoc(
      doc(db, 'palpites', `${usuario.codigo}_${usuario.apelido}`),
      novosPalpites
    );
  }
  ...
  async function handleSalvarMataMata(jogoId, palpite) {
    ...
    await setDoc(
      doc(db, 'palpitesMataMata', `${usuario.codigo}_${usuario.apelido}`),
      novos
    );
  }
  ...
  function handleAdicionarConta(apelido, codigo) {
    ...
    const jaExiste = contas.some(
      (c) => c.apelido === apelido && c.codigo === codigo
    );
    if (jaExiste) return;
    const novasContas = [...contas, { apelido, codigo }];
    const novoIndice = novasContas.length - 1;
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      {tela === 'matamata' && (
        <MataMata
          jogosMataMata={jogosMataMata}
          palpites={palpitesMataMata}
          onSalvar={handleSalvarMataMata}
          faseAtiva={faseAtiva}
          resultados={resultados}
        />
      )}
