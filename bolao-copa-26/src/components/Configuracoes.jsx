...
function Campo({ label, icon, children }) {
  return (
    <div style={{ position: 'relative' }}>
      <div
        style={{ position: 'absolute', left: '14px', top: '8px', zIndex: 1 }}
      >
        <div
          style={{
...
function ModalAdicionarConta({ onAdicionado, onFechar, contasExistentes }) {
  ...
  async function handleEntrar() {
    if (!apelido.trim() || !nomeGrupo.trim() || !senha) {
      dispararShake('Preencha todos os campos.');
      return;
    }
    if (/[A-Z]/.test(apelido.trim())) {
      dispararShake('apenas letras minusculas');
      return;
    }
    if (nomeGrupo.trim() === '__ADMIN__') {
      dispararShake('Conta admin não pode ser adicionada.');
      return;
    }
    if (senha.length !== 6) {
      dispararShake('A senha deve ter 6 dígitos.');
      return;
    }
    // impede duplicata
    ...
    if (jaExiste) {
      dispararShake('Essa conta já está adicionada.');
      return;
    }
    setCarregando(true);
    setErro('');
    try {
      ...
      if (!grupoSnap.exists()) {
        dispararShake('Grupo não encontrado.');
        setCarregando(false);
        return;
      }
      if (grupoSnap.data().senha !== senha) {
        dispararShake('Senha incorreta.');
        setCarregando(false);
        return;
      }
  ...
  async function handleCriar() {
    ...
    if (senhaCriar.length !== 6) {
      dispararShake('A senha deve ter exatamente 6 dígitos.');
      return;
    }
    if (senhaCriar !== confirma) {
      dispararShake('As senhas não coincidem.');
      return;
    }
    const jaExiste = contasExistentes.some(
      (c) =>
        c.apelido === apelidoCriar.trim() && c.codigo === nomeGrupoCriar.trim()
    );
    if (jaExiste) {
      dispararShake('Essa conta já está adicionada.');
      return;
    ...
    try {
      ...
      const grupoSnap = await getDoc(grupoRef);
      if (grupoSnap.exists()) {
        dispararShake('Nome de grupo em uso. Escolha outro.');
        setCarregando(false);
        return;
      }
      await setDoc(grupoRef, {
        nome: nomeGrupoCriar.trim(),
        senha: senhaCriar,
        criadoEm: new Date().toISOString(),
        criadoPor: apelidoCriar.trim(),
      });
  ...
  const senhasOk =
    senhaCriar.length === 6 && confirma.length === 6 && senhaCriar === confirma;
        ...
        >
          {subtela === 'entrar' ? (
            <>
              ...
              >
                {carregando ? 'VERIFICANDO...' : 'ENTRAR NO GRUPO'}
              </button>
            </>
          ) : (
            <>
              ...
              <Campo
                label="NOME DO GRUPO (MÁX. 15 CARACTERES)"
                icon={IconGrupo}
              >
                <input
                  placeholder="Escolha um nome único"
                  value={nomeGrupoCriar}
                  onChange={(e) =>
                    setNomeGrupoCriar(e.target.value.slice(0, 15))
                  }
                  style={inputBase}
                  onFocus={(e) =>
                    (e.target.style.borderColor = 'rgba(201,168,76,0.5)')
              ...
              <Campo label="SENHA DO GRUPO (6 DÍGITOS)" icon={IconLock}>
                <input
                  ...
                  inputMode="numeric"
                  onChange={(e) =>
                    setSenhaCriar(e.target.value.replace(/\D/g, '').slice(0, 6))
                  }
                  style={{ ...inputBase, letterSpacing: '6px' }}
                  onFocus={(e) =>
                    (e.target.style.borderColor = 'rgba(201,168,76,0.5)')
              ...
              <Campo label="CONFIRMAR SENHA" icon={IconLock}>
                <input
                  ...
                  style={{
                    ...inputBase,
                    letterSpacing: '6px',
                    borderColor: senhasDiferentes
                      ? 'rgba(229,57,53,0.5)'
                      : 'rgba(255,255,255,0.1)',
                  }}
                  onFocus={(e) =>
                    (e.target.style.borderColor = senhasDiferentes
                      ? 'rgba(229,57,53,0.5)'
                      : 'rgba(201,168,76,0.5)')
                  }
                  onBlur={(e) =>
      ...
      <div style={{ padding: '16px 14px 0' }}>
        ...
        >
          ...
          >
            ...
            >
              {contaAtivaObj?.apelido?.[0]?.toUpperCase() ?? '?'}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              >
                {contaAtivaObj?.apelido?.toUpperCase()}
              </div>
              <div
                style={{
                  fontFamily: "'Barlow Condensed', sans-serif",
              ...
              >
                {contaAtivaObj?.codigo?.toUpperCase()}
              </div>
            </div>
            <div
              style={{
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <BotaoAcao
              ...
              onClick={() =>
                setConfirmando({ tipo: 'deslogar', conta: contaAtiva })
              }
            />
            <div
              style={{
            ...
            <BotaoAcao
              ...
              onClick={() =>
                setConfirmando({ tipo: 'sair', conta: contaAtiva })
              }
            />
          </div>
        </div>
        ...
        <div style={{ marginTop: outrasContas.length > 0 ? '8px' : '24px' }}>
          <button
            onClick={() => setModalAdicionando(true)}
            style={{
              width: '100%',
  ...
  return (
    <button
      ...
      style={{
        ...
        background: corBg,
        border: 'none',
        borderBottom: `1px solid ${corBorder}`,
        padding: '14px 16px',
        cursor: 'pointer',
        textAlign: 'left',
        display: 'flex',
