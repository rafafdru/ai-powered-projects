    ...
    <>
      <div
        style={{
          ...
          inset: 0,
          background:
            'radial-gradient(ellipse 70% 55% at 50% 38%, rgba(201,168,76,0.13) 0%, rgba(201,168,76,0.04) 40%, transparent 70%)',
          opacity: glow ? 1 : 0.5,
          transition: 'opacity 2s ease',
          pointerEvents: 'none',
        }}
      />
      {[280, 200, 136, 88].map((size, i) => (
        <div
          key={size}
          style={{
            position: 'absolute',
            ...
            border: `1px solid rgba(201,168,76,${0.04 + i * 0.025})`,
            borderRadius: '50%',
            pointerEvents: 'none',
          }}
        />
      ...
      <div
        style={{
          ...
          width: '1px',
          background:
            'linear-gradient(to bottom, transparent 0%, rgba(201,168,76,0.08) 30%, rgba(201,168,76,0.15) 50%, rgba(201,168,76,0.08) 70%, transparent 100%)',
          pointerEvents: 'none',
        }}
      />
    </>
...
function TelaEntrar({ onEntrar, onIrCriar }) {
  ...
  async function handleEntrar() {
    ...
    if (nomeGrupo.trim() === '__ADMIN__') {
      onEntrar('__adminuser__', '__ADMIN__');
      return;
    }
    if (senha.length !== 6) {
...
function TelaCriar({ onEntrar, onVoltar }) {
  ...
  async function handleCriar() {
    if (!apelido.trim() || !nomeGrupo.trim() || !senha || !confirma) {
      dispararShake('Preencha todos os campos.');
      return;
    }
    if (/[A-Z]/.test(apelido.trim())) {
  ...
  const senhasOk =
    senha.length === 6 && confirma.length === 6 && senha === confirma;
      ...
      <Campo label="NOME DO GRUPO (MÁX. 15 CARACTERES)" icon={IconGrupo}>
        ...
        <div
          style={{
            ...
            fontFamily: "'Barlow Condensed', sans-serif",
            fontSize: '9px',
            color: nomeGrupo.length >= 15 ? '#E53935' : 'rgba(255,255,255,0.2)',
          }}
        >
          {nomeGrupo.length}/15
        </div>
...
export default function Login({ onEntrar }) {
  ...
  useEffect(() => {
    const t = setInterval(() => setGlow((g) => !g), 3000);
  ...
  return (
    ...
    >
      ...
      <LogoBloco glow={glow} />
      {tela === 'entrar' && (
        <TelaEntrar onEntrar={onEntrar} onIrCriar={() => setTela('criar')} />
      )}
      {tela === 'criar' && (
        <TelaCriar onEntrar={onEntrar} onVoltar={() => setTela('entrar')} />
      )}
      <style>{`
